...
Render разворачивается по блюпринту [render.yaml](render.yaml): New → Blueprint,
указать репозиторий. Остальное — сборка, health check, автодеплой на коммит в
`main` — описано в файле.

На бесплатном плане постоянный диск не подключается, поэтому SQLite живёт в
файловой системе контейнера и обнуляется при перезапуске сервиса. Чтобы брони
переживали рестарт, нужен диск, смонтированный в `/data`.

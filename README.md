...
npm test                 # 9 сценариев в настоящем браузере
```
...
из этого репозитория, ветка `main`.

Пересборку запускает CI, а не Render: job «Деплой на Render» дёргает deploy hook
после того, как на `main` прошли все проверки. Так на прод не уезжает коммит,
который не собрался или провалил сценарии.
...
| Интеграционные тесты | готовы: 9 сценариев Playwright поверх зафиксированных в `scenarios.md` |
| CI и релизы | готовы: GitHub Actions на каждый push, release-please с changelog |
| Деплой | готов: Render, публичная ссылка, выкладка из CI после зелёных проверок |
